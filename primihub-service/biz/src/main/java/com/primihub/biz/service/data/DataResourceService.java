...
//                continue;
    ...
    public BaseResultEntity handleDataResourceFile(DataResource dataResource, String url) throws Exception {
        ...
        if (file.exists()) {
            ...
            if (resourceFileData.isFileContainsY() && resourceFileData.getResourceFileYRow() > 0) {
    ...
    public ResourceFileData getResourceFileData(String filePath) throws Exception {
        ...
        while ((str = bufferedReader.readLine()) != null) {
                ...
                if (StringUtils.isNotBlank(str)) {
                    ...
                    if (resourceFileData.getFieldSize() == rowValSplit.length) {
                        ...
                        if (StringUtils.isNotBlank(rowValSplit[yIndex]) && !"0".equals(rowValSplit[yIndex])) {
                            resourceFileData.addFileYRowSize();
                        }
                    }
                }
            } else {
            ...
            resourceFileData.addContentSize();
        }
...
//            log.info(dto.getCopyPart());
            ...
            if (BaseUtil.nodeHasGRPCInProxy(baseConfiguration)) {
                for (DataResourceCopyVo vo : dataResourceCopyVos) {
                    String oldAddress = (String) vo.getDataSet().getOrDefault("address", StringUtils.EMPTY);
                    log.info("[address][before] --- [{}]", oldAddress);
                    String[] split = oldAddress.split(SysConstant.COLON_DELIMITER);
                    if (split.length < 3) {
                        continue;
                    ...
                    String replace2 = replace1.replace(oldPort, String.valueOf(outProxy.getPort()));
                    log.info("[address][after] --- [{}]", replace2);
                    vo.getDataSet().put("address", replace2);
                }
            }
